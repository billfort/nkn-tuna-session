...
func main() {
	numTunaListeners := flag.Int("n", 2, "number of tuna listeners")
	numBytes := flag.Int("m", 1, "data to send (MB)")
	numClients := flag.Int("c", 2, "number of clients")
	// FXB
	...
	if *listen {
		...
		go func() {
			for {
				...
				log.Println(c.Addr(), "accepted a session, localAddr is ", s.LocalAddr().String())
